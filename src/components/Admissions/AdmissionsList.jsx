...
const AdmissionsList = () => {
  ...
  const createAdmission = async (data) => {
    try {
      ...
      if (status === 201) toast.success("Admission created successfully!");
  ...
  const save = async (submissionData) => {
    if (admissionToBeUpdated === null) await createAdmission(submissionData);
    else await updateAdmission(submissionData);

    await getAdmissions(faculty.universityId, faculty.id, null);
  };

  useEffect(() => {
    if (faculty?.universityId && faculty?.id) {
