...
const DegreesList = () => {
  ...
  const createDegree = async (data) => {
    try {
      ...
      if (status === 201) toast.success("Degree created successfully!");
  ...
  const save = async (submissionData) => {
    ...
    if (degreeToBeUpdated === null) await createDegree(submissionData);
    else await updateDegree(submissionData);
  ...
  return (
    <>
      {isAddAndUpdateDegreeModalOpen && (
        <AddAndUpdateDegreeModal
          isOpen={isAddAndUpdateDegreeModalOpen}
          onClose={() => {
            setIsAddAndUpdateDegreeModalOpen(false);
            setDegreeToBeUpdated(null);
          }}
          degreeToBeUpdated={degreeToBeUpdated}
          onSave={save}
        />
      )}
      ...
      <div className={styles.header}>
        <div className={styles.headerMain}>
          ...
          <button
            ...
            onClick={() => setIsAddAndUpdateDegreeModalOpen(true)}
          >
            ➕ Add Degree
          </button>
        </div>
      ...
      <div className={styles.tableContainer}>
        {loading ? (
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {degrees.map((degree) => (
                <tr key={degree.id} className={styles.tableRow}>
                  <td className={styles.nameCell}>
                    <div className={styles.facultyName}>{degree.name}</div>
                  </td>
                  <td>
