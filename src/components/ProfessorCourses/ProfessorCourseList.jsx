...
const ProfessorCourseList = ({ professor = null, course = null, onBack }) => {
  ...
  const getAllProfessorCourses = async (facultyId, professorId, courseId) => {
    ...
    try {
      ...
      else toast.error("Something went wrong we could not load professor courses.");
    } catch (error) {
  ...
  const reload = () => {
    if (professor) getAllProfessorCourses(faculty.id, professor.id, null);
    else if (course) getAllProfessorCourses(faculty.id, null, course.id);
    else getAllProfessorCourses(faculty.id);
  };

  const assignProfessorToCourse = async (data) => {
    try {
      const { data: datapc, status } = await professorCourseApi.assignProfessorToCourse(data);
      if (status === 201) toast.success("Course assigned to Professor successfully!");
      else toast.error(datapc?.message ?? "Something went wrong.");
    } catch (error) {
      toast.error(error.message);
    }
  };
  ...
  const updateProfessorCourse = async (data) => {
    ...
    try {
      const { data: datapc, status } = await professorCourseApi.updateProfessorCourse(data);
      if (status === 204) {
        toast.success("Professor Course updated successfully!");
        setProfessorCourseToBeUpdated(null);
      } else {
        toast.error(datapc?.message ?? "Something went wrong.");
      }
    } catch (error) {
      toast.error(error.message);
    }
  ...
  useEffect(() => {
    if (faculty?.id) reload();
    else setLoading(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [faculty?.id, professor?.id, course?.id]);
    ...
    <>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerMain}>
          ...
          {!professor && !course && (
            <h1 className={styles.pageTitle}>Professor Course Assignments</h1>
          )}
        ...
        <div className={styles.headerControls}>
          <div className={styles.searchBar}>
            ...
            <input
              type="text"
              placeholder={`Search ${!professor ? "Professor, " : ""}${!course ? "Course, " : ""}Academic Year`}
              value={searchTerm}
      ...
      <div className={styles.tableContainer}>
        {loading ? (
          ...
          <div className={styles.emptyState}>
            ...
            <p>Try adjusting your search or assign a course to a professor.</p>
          </div>
        ) : (
