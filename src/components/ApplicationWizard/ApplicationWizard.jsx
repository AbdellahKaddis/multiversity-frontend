  ...
  const [formData, setFormData] = useState(
    {
      ...
      dob: toDateInputValue(applicant?.dob), placeOfBirth: applicant?.placeOfBirth, gender: applicant?.gender,
      cin: applicant?.cin, massarCode: applicant?.massarCode, phone: applicant?.phone,
...
if (data.programId && hasAlreadyApplied) {
  ...
  if (isNaN(g) || g < 0 || g > 20)
    e.grade = "Grade must be between 0 and 20.";
}}
    if (key === "photo") {
      if (!data.photo) e.photo = "Profile photo is required.";
    }
    if (key === "documents") {
      if (!data.document)
        e.document = 'Document is required';
    }
    return e;
  };
...
<Link to={'/student/applications'}>        <button className={styles.exitButton} aria-label="Exit wizard">
          ←
        </button></Link>
      </header>
  ...
  <section className={styles.stepPane}>
    ...
    <div className={styles.photoArea}>
      ...
      {showError("photo") && (
        <p className={styles.error}>{errorText("photo")}</p>
      )}
    </div>
  </section>
)}
...
{currentStep.key === "documents" && (
  <section className={styles.stepPane}>
    ...
    <div className={styles.photoArea}>
      ...
      {showError("document") && (
        <p className={styles.error}>{errorText("document")}</p>
      )}
    </div>
  </section>
)}
